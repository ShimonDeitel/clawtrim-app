...
struct ContentView: View {
    ...
    @State private var editingEntry: TrimEntry?
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("No trim entries yet")
                .font(Theme.bodyFont)
                .foregroundStyle(Theme.textSecondary)
        }
    }

    private func entryRow(_ entry: TrimEntry) -> some View {
...
struct EntryEditView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(entry == nil ? "Add Trim" : "Edit Trim")
            .toolbar {
            ...
            .onAppear {
                if let entry {
                    ...
                    f2Text = String(describing: entry.note)
                }
            }
        }
    }
    ...
    private func save() {
        ...
        let result = TrimEntry(id: entry?.id ?? UUID(), date: entry?.date ?? Date(), petName: f1Value, note: f2Value)
        onSave(result)
        dismiss()
    }
}
